...
const VoiceSearchSchema: FC<VoiceSearchSchemaProps> = ({ 
	...
	const speakableSchema = {
		...
		"speakable": {
			...
			"cssSelector": speakableSections.map(section => section.cssSelector),
			"xpath": speakableSections.map(section => `//*[contains(@class, '${section.cssSelector.replace('.', '')}')]`)
		}
	};

	// Enhanced FAQ Schema optimized for voice search
	...
	const howToSchema = {
		...
		"description": "Learn how to use authentic Indonesian sambal in everyday British dishes for amazing flavor enhancement",
		"totalTime": "PT5M",
		"supply": [
			{
				"@type": "HowToSupply",
	...
	return (
		<>
			...
			<script
				type="application/ld+json"
				dangerouslySetInnerHTML={{
					__html: JSON.stringify(voiceFAQSchema, null, 2)
				}}
			/>
			<script
				type="application/ld+json"

...
const STATIC_ASSETS = [
	...
	"/images/logo.webp",
	"/images/favicon.ico",
	// Google Fonts CSS no longer used (fonts loaded via next/font)
];
...
self.addEventListener("install", (event) => {
	...
	event.waitUntil(
		caches
			...
			.catch((error) => {
				console.error(
					"[ServiceWorker] Installation failed:",
					error
				);
			})
	);
				...
				return Promise.all(
					cacheNames
						.filter((cacheName) => {
							...
							return (
								cacheName.startsWith("spice-island-") &&
...
function getCacheStrategy(request, url) {
	...
	for (const [key, strategy] of Object.entries(CACHE_STRATEGIES)) {
		if (
			strategy.urlPattern.test(request.url) ||
			strategy.urlPattern.test(pathname)
		) {
...
async function networkFirst(request, strategy) {
	try {
		...
		console.log(
			"[ServiceWorker] Network failed, trying cache:",
			request.url
		);
		const cache = await caches.open(strategy.cacheName);
		const cached = await cache.match(request);
...
async function staleWhileRevalidate(request, strategy) {
	...
	if (cached) {
		console.log(
			"[ServiceWorker] Serving cached, updating in background:",
			request.url
		);
		return cached;
	}
...
async function cleanupCache(cacheName, maxEntries) {
	...
	if (keys.length > maxEntries) {
		...
		console.log(
			`[ServiceWorker] Cleaned up ${keysToDelete.length} entries from ${cacheName}`
		);
	}
}
